...
describe('Weather app \n', function() {
  ...
  it('selecting a suggestion should change the input field value to the selected city\'s name', async function() {
    $city.sendKeys('dall');
    const responsePromise = new Promise((resolve, reject) => {
      request.get("https://jsonmock.hackerrank.com/api/weather?name=dall",
          (err, response, body) => {
              resolve(body);
          }
      );
  ...
  it('should debounce before making API calls - no result found', async function() {
    ...
    await wait(5000);
    const finalText = await driver.executeScript("return document.getElementsByClassName('suggestionItem')[0].innerText");
  ...
  it('should debounce before making API calls - result should have final call results', async function() {
    ...
    $city.sendKeys('aber');
    const responsePromise = new Promise((resolve, reject) => {
      request.get(`https://jsonmock.hackerrank.com/api/weather?name=aber`,
          (err, response, body) => {
              resolve(body);
          }
      );
  ...
  it('make sure api is hit - test 1', async function() {
    ...
    const responsePromise = new Promise((resolve, reject) => {
      request.get(`https://jsonmock.hackerrank.com/api/weather?name=${randomString}`,
          (err, response, body) => {
              resolve(body);
          }
      );
